# WB-PIML censor-aware, temperature-robust competing-damage results

## Analysis protocol

- Data: `data.xlsx` / `Plot_Data_Verified` only.
...
- `Plot_Data_Verified` is physically deduplicated: 222 unique records and no repeated equivalence ID. Duplicate copies remain traceable in `Plot_Data` and `Excluded_Records` and are excluded from the primary analysis.
